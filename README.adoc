...
OpenStack ofrece el servicio de almacenamiento de bloques, lo que nos permite poder crear volúmenes y conectarlos a máquinas virtuales. Los volúmenes nos dan almacenamiento persistente, entendido como que el volumen se conserva a pesar de que se elimine la máquina virtual a la que está conectado.
...
Tras unos instances el volumen aparecerá en la lista de volúmenes creados y disponible para ser conectado a una instancia.
...
. Selecciona en el formulario la instancia a la conectar el volumen
...
. Introduce un nombre significativo de la transferencia para que pueda ser identificado clarmente por otras personas en el caso de que se vaya a transferir el volumen a otro usuario OpenStack.

+
image::CreateVolumeTransfer[]

. Pulsa el botón `Create Volume Transfer`. Como resultado se nos mostrará un ID de trasnferencia y una clave de autorización que debemos conservar para poder completar el proceso de transferencia del volumen. 
